...
class SceneRepairService {
  ...
  async rebuild(productionId) {
    ...
    await this.videoGenerator.burnCaptionsIntoVideo(finalPath, captionsPath);
    const validatedVideo = await this.videoGenerator.validateVideoFile(finalPath);
    ...
    const assets = {
      ...
      finalVideo: {
        ...(bundle.assets?.finalVideo || {}), path: finalPath, previousPath, fileSize: validatedVideo.bytes || stats.size,
  ...
  async createFallbackSceneImage(productionId, scene, timestamp) {
    const outputPath = path.join(this.dataRoot, 'assets', `${productionId}_scene_${scene.position}_${timestamp}.png`);
    const hue = (Number(scene.position || 0) * 37) % 360;
    await fs.mkdir(path.dirname(outputPath), { recursive: true });
    await sharp({
      create: { width: 1920, height: 1080, channels: 3, background: `hsl(${hue}, 30%, 16%)` }
  ...
  async rebuildNarration(productionId, scenes, fallbackAudioPath, timestamp) {
    ...
    const originalNarrationStillMatches = scenes.every(scene => scene.narrationStatus === 'current' && scene.revision <= 1);
    if (await this.videoGenerator.isUsableAudioFile(fallbackAudioPath) && (allCurrentWithoutSegments || originalNarrationStillMatches)) {
